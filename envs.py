...
class MarioEnvironment(Process):
    ...
    def run(self):
        ...
        while True:
            ...
            if info['x_pos'] > self.max_pos and info['x_pos'] < 3000:
                self.max_pos = info['x_pos']

            if info['x_pos'] >= self.x_rew:
                reward = 15
                self.x_rew += 100
            else:
                reward = 0
            ...
            self.child_conn.send([self.history[:, :, :], r, int(force_done), done, log_reward, max_pos])
